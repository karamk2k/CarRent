...
$('#rentalForm').on('submit', async function(e) {
    ...
    try {
        ...
        if (result.paymentIntent && result.paymentIntent.status === 'succeeded') {
            ...
            window.location.href = `/my-history?success=${encodeURIComponent(confirmResponse.message || 'Rental confirmed successfully')}`;
        } else {
      ...
      ; // Reinitialize CarsGrid to ensure it reflects the latest state
    }
});
            ...
            card.innerHTML = `
                ...
                <div class="p-6">
                    <div class="flex justify-between items-start mb-4">
                        ...
                        <svg class="w-5 h-5 text-white-400" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M16 3H8a2 2 0 00-2 2v16h2v-6h8v6h2V5a2 2 0 00-2-2zm0 8H8V5h8v6zm4-4v8a2 2 0 01-2 2h-1v2h3a1 1 0 001-1v-9h-1z"/>
                        </svg>
                        <span class="ml-1 text-gray-600">${carData.fuel_capacity || '4.5'}</span>
                    </div>
    ...
    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" d="M16 3H8a2 2 0 00-2 2v16h2v-6h8v6h2V5a2 2 0 00-2-2zM16 8h1a1 1 0 011 1v4a2 2 0 002 2"/>
    </svg>
    ${carData.fuel_type || ''}
</div>
                ...
                success: (response) => {
                    if (response.data && response.data.length > 0) {
                        ...
                        $categorySelect.find('option:not([value=""])').remove();
                         const uniqueCategories = [...new Set(categories.map(car => car.category))];
